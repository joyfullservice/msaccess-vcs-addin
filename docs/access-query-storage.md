...
|----------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| Query parameters beyond Long/Text (`Attribute 2`)  | `qryRegressionParameterizedCrosstab` pins Text via the SQL-memo path; `qryRegressionDesignViewParameters` pins Long + Text and `qryRegressionDesignViewParameterTypes` pins Boolean, DateTime, Currency + Double (plus an unbracketed parameter name) through the Design View `Begin Parameters` block. `ParameterFlagFromType` maps the remaining DAO flags Riddington documents (Byte, Integer/Short, Single, OLE, Memo, GUID, BigInt), but those are not yet exercised by a round-trip fixture; add fixtures for them if observed in the wild. |
