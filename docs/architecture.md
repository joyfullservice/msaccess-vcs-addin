...
| `Run_BuildAndInstall` (`VCS.RebuildAddIn`) | The add-in cannot rebuild and reinstall itself while loaded. Unattended callers poll `logs/rebuild-status.json` after Access exits. Refuses unless this is the only `MSACCESS.EXE` in the session, and never closes another process. See [agentic-rebuild.md](agentic-rebuild.md). |
