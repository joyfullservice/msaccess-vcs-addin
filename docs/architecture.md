...
| `Run_BuildAndInstall` (`VCS.RebuildAddIn`) | The add-in cannot rebuild and reinstall itself while loaded. Unattended callers poll `logs/rebuild-status.json` after Access exits. Refuses when another `MSACCESS.EXE` in the session has one of the files it replaces loaded as a VBA project, and never closes another process. See [agentic-rebuild.md](agentic-rebuild.md). |
