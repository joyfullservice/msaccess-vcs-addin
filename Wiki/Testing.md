...
`VCS.RunTests` uses two static-discovery paths. Its **native `TestAssert` convention** does not use a `@Test` attribute; the separate, opt-in Rubberduck path recognizes `@TestModule` / `@TestMethod` annotations. See [Rubberduck Testing Support](Rubberduck-Testing-Support) for its migration, compatibility, and execution guidance.
